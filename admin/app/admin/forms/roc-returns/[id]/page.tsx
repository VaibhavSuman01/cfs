...
export default function ROCReturnsDetailPage() {
  ...
  const [form, setForm] = useState<ROCForm | null>(null)
  ...
  const [newStatus, setNewStatus] = useState<ROCForm['status']>('Pending')
  ...
  const fetchFormDetails = async () => {
    try {
      ...
      const targetForm = forms.find((f: any) => f._id === formId && f.formType === 'ROCForm')
      ...
      if (targetForm) {
        ...
        router.push('/admin/forms/roc-returns')
      }
    } catch (error) {
  ...
  const handleSendReport = async () => {
    ...
    try {
      ...
      formData.append('reportType', 'ROC Returns Report');
      ...
      const response = await api.get(`${API_PATHS.ADMIN.SERVICE_FORMS}`)
      const forms = response.data.forms
      const targetForm = forms.find((f: any) => f._id === form._id && f.formType === 'ROCForm')
    ...
    return (
      <div className="container mx-auto p-6">
        <div className="text-center">
          ...
          <Button onClick={() => router.push('/admin/forms/roc-returns')} className="mt-4">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <Button
              variant="outline"
              onClick={() => router.push('/admin/forms/roc-returns')}
            >
              <ArrowLeft className="mr-2 h-4 w-4" />
              Back
            </Button>
            <div>
              <h1 className="text-3xl font-bold text-white">ROC Returns Form</h1>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm">{form.companyType || 'Not provided'}</p>
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-sm flex items-start gap-2">
                  ...
                  {form.companyAddress || 'Not provided'}
                </p>
              </div>
