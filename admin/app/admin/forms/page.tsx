...
import { Loader2, Search, FileText, MoreHorizontal, CheckCircle, XCircle, Clock } from 'lucide-react';
...
export default function FormsPage() {
  ...
  const [newStatus, setNewStatus] = useState<'Pending' | 'Reviewed' | 'Filed'>('Pending');
  ...
  const filteredForms = forms.filter(form => {
    ...
    return (
      form.userId.name.toLowerCase().includes(searchLower) ||
      form.userId.email.toLowerCase().includes(searchLower) ||
      form.taxYear.includes(searchQuery) ||
      form.incomeType.toLowerCase().includes(searchLower)
    );
  });
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="relative w-full max-w-sm">
          ...
          <Input
            placeholder="Searc variant={contact.isResolved ? 'secondary' : 'outline'}>h forms..."
            className="pl-8"
            value={searchQuery}
                    ...
                    <TableRow key={form._id}>
                      <TableCell>
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">{form.userId.email}</div>
                        </div>
                      </TableCell>
                      <TableCell>{form.taxYear}</TableCell>
                      <TableCell>{form.incomeType.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}</TableCell>
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuSeparator />
                            <DropdownMenuItem onClick={() => openFormDetails(form)}>
            ...
            <div className="grid gap-6 py-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-base font-medium">{selectedForm.userId.name}</p>
                  <p className="text-sm text-muted-foreground">{selectedForm.userId.email}</p>
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-base">{selectedForm.incomeType.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-base">{formatCurrency(selectedForm.grossIncome)}</p>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-muted-foreground">Deductions</h3>
                  <p className="text-base">
                    {selectedForm.deductions ? formatCurrency(selectedForm.deductions) : 'None'}
                  </p>
                </div>
              </div>
              ...
              {selectedForm.additionalNotes && (
                <div>
                  ...
                  <p className="text-base whitespace-pre-line">{selectedForm.additionalNotes}</p>
                </div>
              )}
