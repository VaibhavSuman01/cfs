...
export default function TrademarkISODetailPage() {
  ...
  const [form, setForm] = useState<TrademarkISOForm | null>(null)
  ...
  const [newStatus, setNewStatus] = useState<TrademarkISOForm['status']>('Pending')
  ...
  const fetchFormDetails = async () => {
    try {
      ...
      const targetForm = forms.find((f: any) => f._id === formId && f.formType === 'TrademarkISOForm')
      ...
      if (targetForm) {
        ...
        router.push('/admin/forms/trademark-iso')
      }
    } catch (error) {
  ...
  const handleSendReport = async () => {
    ...
    try {
      ...
      formData.append('reportType', 'Trademark & ISO Report');
      ...
      const response = await api.get(`${API_PATHS.ADMIN.SERVICE_FORMS}`)
      const forms = response.data.forms
      const targetForm = forms.find((f: any) => f._id === form._id && f.formType === 'TrademarkISOForm')
    ...
    return (
      <div className="container mx-auto p-6">
        <div className="text-center">
          ...
          <Button onClick={() => router.push('/admin/forms/trademark-iso')} className="mt-4">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <Button
              variant="outline"
              onClick={() => router.push('/admin/forms/trademark-iso')}
            >
              <ArrowLeft className="mr-2 h-4 w-4" />
              Back
            </Button>
            <div>
              <h1 className="text-3xl font-bold text-white">Trademark & ISO Form</h1>
