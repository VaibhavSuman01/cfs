...
const StatusBadge = ({ status }: { status: TrademarkISOForm['status'] }) => {
  const statusConfig = {
    Pending: {
      variant: 'secondary',
...
export default function TrademarkISOPage() {
  ...
  const [forms, setForms] = useState<TrademarkISOForm[]>([]);
  ...
  const fetchTrademarkISOForms = useCallback(async () => {
    setLoading(true);
    try {
      ...
      // Send service filter to backend (Trademark & ISO)
      params.append('service', 'Trademark & ISO');
      ...
      let trademarkISOForms = response.data.forms.filter((form: any) => form.formType === 'TrademarkISOForm');
      
      // Filter by subService if selected
      if (subService && subService !== 'all') {
        trademarkISOForms = trademarkISOForms.filter((form: any) => {
      ...
      setForms(trademarkISOForms);
      setPagination({
        total: trademarkISOForms.length,
        page: page,
        limit: 10,
        pages: Math.ceil(trademarkISOForms.length / 10)
      });
    } catch (error) {
      console.error('Failed to fetch trademark ISO forms:', error);
      toast({
        title: 'Error',
        description: 'Failed to load trademark ISO forms.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
    ...
    <div className="container mx-auto p-6">
      ...
      >
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white">Trademark & ISO Forms</h1>
            <p className="text-muted-foreground text-white">Manage and review all trademark and ISO applications.</p>
          </div>
        </div>
              ...
              <div className="w-full md:w-1/4">
                <Select onValueChange={handleSubServiceChange} value={subService || 'all'}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="Trademark Search">Trademark Search</SelectItem>
                    <SelectItem value="Trademark Opposition">Trademark Opposition</SelectItem>
                    <SelectItem value="Trademark Renewal">Trademark Renewal</SelectItem>
                    <SelectItem value="Trademark Assignment">Trademark Assignment</SelectItem>
                    <SelectItem value="Trademark Licensing">Trademark Licensing</SelectItem>
                    <SelectItem value="ISO 9001 Certification">ISO 9001 Certification</SelectItem>
                    <SelectItem value="ISO 14001 Certification">ISO 14001 Certification</SelectItem>
                    <SelectItem value="ISO 45001 Certification">ISO 45001 Certification</SelectItem>
                    <SelectItem value="ISO 27001 Certification">ISO 27001 Certification</SelectItem>
                    <SelectItem value="ISO 22000 Certification">ISO 22000 Certification</SelectItem>
                    <SelectItem value="ISO 13485 Certification">ISO 13485 Certification</SelectItem>
                    <SelectItem value="ISO 20000 Certification">ISO 20000 Certification</SelectItem>
                    <SelectItem value="ISO 50001 Certification">ISO 50001 Certification</SelectItem>
                    <SelectItem value="ISO 17025 Certification">ISO 17025 Certification</SelectItem>
                    <SelectItem value="ISO 15189 Certification">ISO 15189 Certification</SelectItem>
                    <SelectItem value="ISO 22000 Certification">ISO 22000 Certification</SelectItem>
                    <SelectItem value="ISO 13485 Certification">ISO 13485 Certification</SelectItem>
                    <SelectItem value="ISO 20000 Certification">ISO 20000 Certification</SelectItem>
                    <SelectItem value="ISO 50001 Certification">ISO 50001 Certification</SelectItem>
                    ...
                    <TableRow key={form._id}>
                      ...
                      <TableCell>
                        <Button 
                          variant="outline" 
                          size="sm"
                          onClick={() => router.push(`/admin/forms/trademark-iso/${form._id}`)}
                        >
