...
export default function AdvisoryDetailPage() {
  ...
  const [form, setForm] = useState<AdvisoryForm | null>(null)
  ...
  const [newStatus, setNewStatus] = useState<AdvisoryForm['status']>('Pending')
  ...
  const fetchFormDetails = async () => {
    try {
      ...
      const targetForm = forms.find((f: any) => f._id === formId && f.formType === 'AdvisoryForm')
      ...
      if (targetForm) {
        ...
        router.push('/admin/forms/advisory')
      }
    } catch (error) {
  ...
  const handleSendReport = async () => {
    ...
    try {
      ...
      formData.append('reportType', 'Advisory Report');
      ...
      const response = await api.get(`${API_PATHS.ADMIN.SERVICE_FORMS}`)
      const forms = response.data.forms
      const targetForm = forms.find((f: any) => f._id === form._id && f.formType === 'AdvisoryForm')
    ...
    return (
      <div className="container mx-auto p-6">
        <div className="text-center">
          ...
          <Button onClick={() => router.push('/admin/forms/advisory')} className="mt-4">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <Button
              variant="outline"
              onClick={() => router.push('/admin/forms/advisory')}
            >
              <ArrowLeft className="mr-2 h-4 w-4" />
              Back
            </Button>
            <div>
              <h1 className="text-3xl font-bold text-white">Advisory Form</h1>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm">{form.industry || 'Not provided'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-muted-foreground">Advisory Area</label>
                  <p className="text-sm">{form.advisoryArea || 'Not provided'}</p>
                </div>
              </div>
              <div>
