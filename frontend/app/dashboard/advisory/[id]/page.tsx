...
export default function AdvisoryDetailPage() {
  ...
  const [form, setForm] = useState<AdvisoryFormDetail | null>(null);
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      try {
        ...
        console.error("Failed to fetch advisory detail", e);
      } finally {
        setLoading(false);
      }
    };
  ...
  const getDocTitle = (d: AdvisoryDocument) => d.originalName || d.fileName || d.filename || d.name || d.documentId || d._id || "Document";
  const getDocId = (d: AdvisoryDocument) => d.documentId || d._id || "";

  const handleDownload = async (doc: AdvisoryDocument) => {
    const documentId = getDocId(doc);
    if (!documentId) return;
    const filename = getDocTitle(doc);
    const url = `/api/forms/advisory/download/${documentId}`;
    ...
    <div className="container mx-auto p-6 max-w-7xl">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Advisory Service Details</h1>
            <p className="text-gray-600 mt-1">Track your advisory request progress and communications</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {getStatusBadge(form.status)}
          <Button 
            onClick={() => router.push(`/dashboard/advisory/${form._id}/edit`)}
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-500">Advisory Type</p>
                      <p className="text-lg font-semibold text-gray-900">{form.advisoryType || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-gray-900">{form.expectedResponse || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
