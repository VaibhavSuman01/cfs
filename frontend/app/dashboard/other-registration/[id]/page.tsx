...
import { Loader2, ArrowLeft, Download, CheckCircle, MessageSquare, FileText, Calendar, User, Building, Clock, MapPin } from "lucide-react";
...
export default function OtherRegistrationDetailPage() {
  ...
  const [form, setForm] = useState<ORFormDetail | null>(null);
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      try {
        setLoading(true);
        const res = await api.get(API_PATHS.FORMS.OTHER_REGISTRATION_DETAIL(id));
        setForm(res.data?.data || res.data);
      } catch (e) {
        console.error("Failed to fetch other registration detail", e);
      } finally {
        setLoading(false);
      }
    };
  ...
  const getDocTitle = (d: ORDocument) => d.originalName || d.fileName || d.filename || d.name || d.documentId || d._id || "Document";
  const getDocId = (d: ORDocument) => d.documentId || d._id || "";

  const handleDownload = async (doc: ORDocument) => {
    const documentId = getDocId(doc);
    if (!documentId) return;
    const filename = getDocTitle(doc);
    const url = `/api/forms/other-registration/download/${documentId}`;
    ...
    <div className="container mx-auto p-6 max-w-7xl">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Other Registration Details</h1>
            <p className="text-gray-600 mt-1">Track your registration application progress and communications</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {getStatusBadge(form.status)}
          <Button 
            onClick={() => router.push(`/dashboard/other-registration/${form._id}/edit`)}
            ...
            <CardContent className="p-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-gray-900">{form.businessName || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-gray-900">{form.registrationType || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-2 h-2 bg-purple-500 rounded-full mt-2"></div>
                    <div>
                      <p className="text-sm font-medium text-gray-500">Registration Purpose</p>
                      <p className="text-lg font-semibold text-gray-900">{form.registrationPurpose || "Not specified"}</p>
                    </div>
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-gray-900">{form.city || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-gray-900">{form.state || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-gray-900">{form.pincode || "Not specified"}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {form.businessAddress && (
                <div className="mt-4">
                  ...
                  <p className="text-gray-900 leading-relaxed">{form.businessAddress}</p>
                </div>
              )}
            </CardContent>
          </Card>
