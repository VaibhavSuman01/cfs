...
export default function ROCReturnsListPage() {
  const { user, isLoading } = useAuth();
  const [forms, setForms] = useState<ROCFormSummary[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const res = await api.get(API_PATHS.FORMS.ROC_RETURNS_USER_SUBMISSIONS);
        const data = (res.data?.data || []) as ROCFormSummary[];
        setForms(data);
      } catch (e) {
        console.error("Failed to load ROC returns submissions", e);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold">My ROC Returns Submissions</h1>
            ...
            <div className="space-y-4">
              {forms.map((f) => (
                <div key={f._id} className="flex items-center justify-between border rounded-lg p-4">
                  <div>
                    <div className="font-semibold">{f.companyName || f.service || "ROC Returns"}</div>
                  ...
                  <div className="flex gap-2">
                    <Button variant="secondary" size="sm" asChild>
                      <Link href={`/dashboard/roc-returns/${f._id}`}>
                        <Eye className="mr-2 h-4 w-4" /> View
                      </Link>
                    </Button>
                    <Button size="sm" asChild>
                      <Link href={`/dashboard/roc-returns/${f._id}/edit`}>
