...
import { Loader2, ArrowLeft, Download, CheckCircle, MessageSquare, FileText, Calendar, User, Building, Clock, BarChart3 } from "lucide-react";
...
export default function ReportsDetailPage() {
  ...
  const [form, setForm] = useState<ReportsFormDetail | null>(null);
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      try {
        setLoading(true);
        const res = await api.get(API_PATHS.FORMS.REPORTS_DETAIL(id));
        setForm(res.data?.data || res.data);
      } catch (e) {
        console.error("Failed to fetch reports detail", e);
      } finally {
        setLoading(false);
      }
    };
  ...
  const getDocTitle = (d: ReportsDocument) => d.originalName || d.fileName || d.filename || d.name || d.documentId || d._id || "Document";
  const getDocId = (d: ReportsDocument) => d.documentId || d._id || "";

  const handleDownload = async (doc: ReportsDocument) => {
    const documentId = getDocId(doc);
    if (!documentId) return;
    const filename = getDocTitle(doc);
    const url = `/api/forms/reports/download/${documentId}`;
    ...
    <div className="container mx-auto p-6 max-w-7xl">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Reports Details</h1>
            <p className="text-gray-600 mt-1">Track your report request progress and communications</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {getStatusBadge(form.status)}
          <Button 
            onClick={() => router.push(`/dashboard/reports/${form._id}/edit`)}
            ...
            <CardContent className="p-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-gray-900">{form.reportType || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-gray-900">{form.subject || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-gray-900">{form.priority || "Not specified"}</p>
                    </div>
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-gray-900">{form.dueDate || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-gray-900">{form.businessType || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-gray-900">{form.projectDetails || "Not specified"}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {form.description && (
                <div className="mt-6 pt-6 border-t">
                  ...
                  <p className="text-gray-900 leading-relaxed">{form.description}</p>
                </div>
              )}
            </CardContent>
          </Card>
